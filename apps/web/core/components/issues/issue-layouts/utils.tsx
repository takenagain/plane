...
const getStateColumns = ({ projectId }: TGetColumns): IGroupByColumn[] | undefined => {
  ...
  const states = projectId ? getProjectStates(projectId) : projectStates;
  if (!states) return;
...
export const getScopeMemberIds = ({ isWorkspaceLevel, projectId }: TGetColumns): TGetScopeMemberIdsResult => {
  ...
  if (projectId || (projectMemberIds && projectMemberIds.length > 0)) {
    ...
    const resolvedProjectMemberIds = projectId ? getProjectMemberIds(projectId, false) : projectMemberIds;
    return {
      memberIds: resolvedProjectMemberIds ?? [],
      includeNone: true,
    };
  }
