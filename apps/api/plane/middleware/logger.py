...
class APITokenLogMiddleware:
    ...
    def _token_identifier(self, api_key: str) -> str:
        """
        Returns a non-sensitive database identifier for the API token, or a
        redacted placeholder when the key cannot be resolved. The raw key is
        never persisted or hashed.
        """
        token_id = APIToken.objects.filter(token=api_key).values_list("id", flat=True).first()
        return str(token_id) if token_id else "[REDACTED]"
